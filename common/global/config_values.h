...
#include "common/uitls/msg_types.h"
...
public:
   ...
   */
  set_value_result_t set_value(const std::string& key, Option::value_t&& value, int level);
   ...
   *              highest-priority
   */
  std::pair<Option::value_t, bool> get_value(const std::string& name, int level) const;
