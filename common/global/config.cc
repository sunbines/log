  ...
  for (const auto &i : schema) {
    ...
    if (opt.type == Option::TYPE_STR) {
      ...
      if (opt.pre_validate(&val, &err) != 0) {
        std::cerr << "Default value " << opt.name << "=" << *def_str << " is invalid: " << err << std::endl;
...
int md_config_t::parse_config_files(ConfigValues& values, const ConfigTracker& tracker, 
                                    ...
                                    std::ostream *warnings, int flags)
{
  ...
  while (p != conf_files.end()) {
    string &s = *p;
    if (s.find("$data_dir") != string::npos && data_dir_option.empty()) {
...
int md_config_t::parse_argv(ConfigValues& values, const ConfigTracker& tracker,
...
void md_config_t::early_expand_meta(const ConfigValues& values,
                                    std::string &val, std::ostream *err) const
{
  expand_stack_t stack;
...
void md_config_t::update_legacy_val(ConfigValues& values, const Option &opt,
